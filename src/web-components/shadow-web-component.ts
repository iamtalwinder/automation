...
export class ShadowWebComponent extends AbstractWebComponent {
  ...
  public async getElements(implicitTimeout: number = 500): Promise<WebElement[]> {
    return await this.browser.wait(
      async () => {
        const elements: WebElement[] = await (await this.getShadowRoot())
        .findElements(this.locator).catch(async () => []);
    ...
    return this.browser.wait(async () => {
      if (!this.secondShadowRootSelector) {
        if (!this.extraSelector) {
          return this.driver
          .executeScript(`return document.querySelector('${this.shadowRootSelector}').shadowRoot`)
          .catch(async () => null);
        } else {
          return this.driver
          .executeScript(`return document.querySelector('${this.shadowRootSelector}').shadowRoot.querySelector('${this.extraSelector}')`)
          .catch(async () => null);
        }
      } else {
        return this.driver
        .executeScript(`return document.querySelector('${this.shadowRootSelector}').shadowRoot.querySelector('${this.secondShadowRootSelector}').shadowRoot`)
        .catch(async () => null);
      }
    });
  }
