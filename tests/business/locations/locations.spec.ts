...
describe('Business Locations', async () => {
  ...
  it('1. Add Locations Successfully', async () => { 
    await locationsPage.addLocation(locationName);
    const location: WebComponent = await locationsPage.getLocationByName(locationName);
    expect(await location.isLocated()).to.be.true;
  ...
  it('3. Should open archive location dialog box', async () => {
    await locationDetailPage.openArchiveLocationDialogBox();
    expect(await locationDetailPage.DialogBoxArchiveLocationButton.isLocated()).to.be.true;
  });

  it('4. Should Archive Location', async () => {
